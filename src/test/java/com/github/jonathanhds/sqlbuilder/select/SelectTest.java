...
public class SelectTest {
    ...
    public void selectAllFromTable() throws Exception {
        List<Person> persons =
            new QueryBuilderHSQLDB(connection).select().all().from().table("PERSON p").list(new PersonRowMapper());

        assertThat(persons, hasSize(3));
        assertThat(persons,
            containsInAnyOrder(jonathan().setCountry(null), steveJobs().setCountry(null), domPedro().setCountry(null)));
    }
    ...
    public void selectColumnsFromTableJoinAnotherTable() throws Exception {
        ...
        persons = new QueryBuilderHSQLDB(connection).select()
            .columns("p.name", "p.birthday", "p.country_id", "c.country_name").from().tables("PERSON p", "COUNTRY c")
            .where("c.id = p.country_id").list(new PersonRowMapper());
    ...
    public void selectAllFromTableGroupBy() throws Exception {
        List<Integer> counts = new QueryBuilderHSQLDB(connection).select().count("*").from().table("PERSON p").groupBy()
            .column("p.birthday").list(new CountRowMapper());
        ...
        counts =
            new QueryBuilderHSQLDB(connection).select().count("c.country_name").from().tables("PERSON p", "COUNTRY c")
                .where("p.country_id = c.id").groupBy("c.country_name").list(new CountRowMapper());
