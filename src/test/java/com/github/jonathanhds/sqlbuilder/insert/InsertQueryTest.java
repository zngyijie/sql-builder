...
public class InsertQueryTest {
    ...
    public void shouldCreateQueryPassingColumnsAndValues() throws Exception {
        // Given
        Insert query = new QueryBuilder(Database.HSQLDB, connection).insert("person").columns("id", "name", "age")
            .values(1, "foo", 30);
        ...
        // Then
        String expected = new StringBuilder("INSERT INTO person").append(NEW_LINE).append(" ( ").append("id, name, age")
            .append(" )").append(NEW_LINE).append("VALUES (").append("1, 'foo', 30").append(")").toString();
    ...
    public void shouldCreateQueryPassingColumnsAndManyValues() throws Exception {
        // Given
        Insert query = new QueryBuilder(Database.HSQLDB, connection).insert("person").columns("id", "name", "age")
            .values(1, "foo", 30).values(2, "bar", 23).values(3, "hello", 54).values(4, "world", 19);
    ...
    public void shouldThrowErrorIfNoValuesIfPassed() throws Exception {
        // Given
        Insert query = new QueryBuilder(Database.HSQLDB, connection).insert().table("person").values(1, "foo", 30);
    ...
    public void shouldThrowErrorIfColumnsSizeIsDifferentOfValuesSize() throws Exception {
        // Given
        Insert query = new QueryBuilder(Database.HSQLDB, connection).insert("person").columns("id", "name", "age")
            .values(1, "bar", 20).values(2, "foo", 30, "bar").values(3, "hello", 45);
