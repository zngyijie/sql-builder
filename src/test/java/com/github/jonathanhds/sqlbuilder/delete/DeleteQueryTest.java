...
public class DeleteQueryTest {
    ...
    public void shouldCreateQuery() throws Exception {
        // Given
        Delete query = new QueryBuilder(Database.HSQLDB, connection).delete("person p");
    ...
    public void shouldCreateQueryUsingWhere() throws Exception {
        // Given
        Delete query = new QueryBuilder(Database.HSQLDB, connection).delete("account a").where("a.id > 666")
        ...
        // Then
        String expected = new StringBuilder("DELETE FROM account a").append(newLine).append("WHERE a.id > 666")
            .append(newLine).append("AND a.creation_date > '2013-01-01'").toString();
        assertEquals(expected, actual);
    }
}
