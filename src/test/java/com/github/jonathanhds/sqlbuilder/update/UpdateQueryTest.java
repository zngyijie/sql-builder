...
public class UpdateQueryTest {
    ...
    public void shouldCreateQueryForTableUsingSets() throws Exception {
        // Given
        Update query = new QueryBuilder(Database.HSQLDB, connection).update("person p").set("p.name", "foo");

        // When
        String actual = query.toString();

        // Then
        String expected = new StringBuilder("UPDATE person p SET").append(newLine).append("p.name = 'foo'").toString();
    ...
    public void shouldCreateQueryUsingWhere() throws Exception {
        // Given
        Update query = new QueryBuilder(Database.HSQLDB, connection).update("employee e").set("e.salary", "50000")
            .where("e.age > 40").where("e.genre = 'female'");
        ...
        // Then
        String expected =
            new StringBuilder("UPDATE employee e SET").append(newLine).append("e.salary = '50000'").append(newLine)
                .append("WHERE ").append("e.age > 40").append(newLine).append("AND e.genre = 'female'").toString();
